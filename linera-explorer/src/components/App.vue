  ...
  <div>
    ...
    <div class="container pb-5">
      ...
      <div v-else-if="page.home">
        <v-block v-if="page.home.blocks.length!=0" :block="page.home.blocks[0]" title="Head">
        </v-block>
        <div class="card">
          <div class="card-header">Blocks</div>
          <div class="card-body">
            <v-blocks :blocks="page.home.blocks"></v-blocks>
          </div>
        </div>
        <div class="card">
          <div class="card-header">Applications</div>
          <div class="card-body">
            <v-applications :apps="page.home.apps"></v-applications>
          </div>
        </div>
      </div>

      <div v-else-if="page.blocks">
        <v-blocks :blocks="page.blocks"></v-blocks>
      </div>

      <div v-else-if="page.block">
        <v-block :block="page.block" title="Block">
        </v-block>
      </div>

      <div v-else-if="page.applications">
        <v-applications :apps="page.applications"></v-applications>
      </div>

      <div v-else-if="page.application">
        <v-application :app="page.application"></v-application>
      </div>

      <div v-else-if="page.operations">
        <v-operations :operations="page.operations"></v-operations>
      </div>

      <div v-else-if="page.operation">
        <v-operation :op="page.operation" :id="operation_id(page.operation.key)" :index="page.operation.index"></v-operation>
      </div>

      <div v-else-if="page.plugin">
        <v-plugin :plugin="page.plugin"></v-plugin>
      </div>
