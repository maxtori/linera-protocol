...
const props = defineProps<{entry: IntrospectionField, link: string, kind: string}>()

const result = ref(undefined)
const errors = ref(undefined)

function query(args: any, kind: string) {
  rust_query({result, errors, ...props}, args, kind)
}

function empty_response(t: any) : any {
  return rust_empty(t)
}
</script>
