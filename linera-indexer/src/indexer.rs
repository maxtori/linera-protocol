...
async fn load_indexer(config: &IndexerConfig) -> Result<Indexer<Context>, IndexerError> {
    let client = RocksDbClient::new(&config.storage, config.cache_size);
    let plugins: Vec<&str> = config.plugins.split(',').collect();
...
async fn server(config: &IndexerConfig, indexer: &Indexer<Context>) -> Result<(), IndexerError> {
    ...
    let app = match operations {
        None => app,
        Some(plugin) => {
            let schema = Schema::build(plugin, EmptyMutation, EmptySubscription).finish();
            app.route("/operations", get(graphiql).post(operations_handler))
...
async fn main() -> Result<(), IndexerError> {
    ...
    info!("config: {:?}", config);
    let indexer = Arc::new(load_indexer(&config).await?);
